Shader "Unlit/Tone Based Shading"
{
    Properties
    {
        ...
        _WarmColor("Warm Colorr", Color) = (1,1,1,1)
        _CoolScale("Cool Scale", Range(-1, 1)) = 0.1
        _WarmScale("Warm Scale", Range(-1, 1)) = 0.1
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float4 kCool = _CoolColor + texColor*_CoolScale;

                float4 kWarm = _WarmColor + texColor*_WarmColor;

                float3 diff2 = (1 + dot(normalDirection, worldLightDir)) * 0.5 * kCool+ (1 + dot(normalDirection, worldLightDir)) * 0.5 * kWarm;
                //渐变纹理采样漫反射
                fixed3 diffuse = diff2;
                ...
                fixed3 specular = (0,0,0);//_LightColor0.rgb * _Specular.rgb * spec * texColor;
