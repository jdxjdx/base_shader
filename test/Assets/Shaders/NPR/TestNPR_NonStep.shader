    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                //渐变纹理采样漫反射
                fixed3 diffuse = _LightColor0.rgb * texColor * _Diffuse.rgb * tex2D(_Ramp, float2(diff, diff)).rgb;
                ...
                //spec = step(_Threshold, spec);
                float w = 0.0000000001;
                spec = lerp(0, 1, smoothstep(-w, w, spec - _Threshold));
                
                //fixed3 specular = _LightColor0.rgb * _Specular.rgb * pow(saturate(dot(normalDirection, halfDir)),_Gloss) * texColor;
                fixed3 specular = _LightColor0.rgb * _Specular.rgb * pow(spec, _Gloss) * texColor;
