Shader "Unlit/TestNPR_1"
{
    Properties
    {
        ...
        _DiffuseCutLocation("DiffuseCutLocation", Float) = 1.0
        _DiffuseCutSmoothness("DiffuseCutSmoothness", Float) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                ///halfLambat
                half NdotL = (dot(normalDirection, worldLightDir)*0.5 + 0.5);

                half diff = min(NdotL, atten);// attenuation为shadowmap计算的阴影
                ...
                half diffuseColor = smoothstep(diffuseMin, diffuseMax, diff) * texColor;
